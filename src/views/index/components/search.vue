...
import { appSource, webSource } from './search.data'
const searchStore = useSearchStore()
const indexStore = useIndexStore()
const { tabIndex } = storeToRefs(indexStore)
const { searchFocus, searchHistory, showSearchHistory, openMode, showAppSource, currentWebIndex } =
...
const handleFocus = () => {
  searchStore.setSearchFocus(true)
  tabIndex.value = 0
}
...
const handleOutsideClick = (e) => {
  ...
  setTimeout(() => {
    if (!isSearchRelated) {
      searchStore.setSearchFocus(false)
    }
  }, 100)
}
  ...
  >
    ...
    >
      ...
      <template v-if="handleValue">
        <div v-if="showAppSource" class="text-sm font-medium text-gray-700 mb-3 flex items-center">
          <svg
        ...
        <div class="flex flex-wrap gap-2 mb-2" v-if="showAppSource">
          <div
            v-for="item in appSource"
            :key="item.type"
            class="text-sm px-3 py-1.5 rounded-xl bg-blue-300 text-white cursor-pointer transition-all duration-200 hover:bg-blue-400 hover:shadow-md transform hover:-translate-y-0.5"
            @click="goSearch(item)"
          >
            {{ item.type }}
          </div>
      ...
      <!-- 搜索历史 -->
      <div class="" v-else-if="showSearchHistory && searchHistory.length">
        ...
        <div class="flex flex-wrap gap-2.5">
          <div
            ...
            :key="index"
            class="bg-blue-50 px-3.5 py-1.75 rounded-lg cursor-pointer hover:bg-blue-100 whitespace-nowrap transition-all duration-200 text-sm max-w-48 group relative flex"
          >
